...
class GaussianDensityTorch(object):
    ...
    def fit(self, embeddings, device):
        self.mean = torch.mean(embeddings, axis=0).to(device)
        self.inv_cov = torch.Tensor(LedoitWolf().fit(embeddings.cpu()).precision_, device='cpu').to(device)
    ...
    def mahalanobis_distance(
        ...
        dist = torch.einsum("im,mn,in->i", x_mu.to(torch.float), inv_covariance.to(torch.float), x_mu.to(torch.float))
        return dist.sqrt()